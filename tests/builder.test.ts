...
test("builder bundles CommonJS dependencies but rejects unresolved require calls", async (t) => {
  ...
  await assert.rejects(buildIntegration(directory), /require.*will not be bundled/);
});
