...
test("all-features example verifies, paginates, checkpoints, and resumes", async (t) => {
  ...
  assert.deepEqual(await runSync(integration, "contacts", { connectionConfig, syncConfig }, host), {
    batches: 2,
    records: 3,
    checkpoint: { pagination: { offset: 3 } },
  });
  const firstEvents = await runSync(integration, "events", { connectionConfig, syncConfig }, host);
