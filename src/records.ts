...
/** Batch record-only snapshots; checkpoint-bearing batches must remain explicit. */
export async function* batchRecords<R>(source: AsyncIterable<R> | Iterable<R>) {
  let records: R[] = [];
  let bytes = 0;

  for await (const record of source) {
    // Validate before copying: JSON.stringify must not silently drop invalid values.
    const snapshot = jsonSnapshot(record, "Batch records must be JSON-compatible");
    const size = new TextEncoder().encode(JSON.stringify(snapshot)).length;

    if (records.length && (records.length >= 100 || bytes + size > 1024 * 1024)) {
      yield { records };
      records = [];
      bytes = 0;
    }
