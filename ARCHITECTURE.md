...
| `@beetlio/connect/host`    | Provider requests, sync execution, verification, OAuth, local file sinks      |
...
integration.ts + package.json + lockfile
    ...
    → withIntegration → runSync / verifyConnection
...
- **Integration:** provider endpoints, response schemas, pagination, record mapping,
  sync mode, and checkpoint meaning. Credentials stay outside sync contexts.
...
The CLI composes the provider and executor with `withFileSink`. The process adapter
...
keeps destination credentials and validates process output independently.
